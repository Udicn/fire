...
const char* mqttwheatherTopic="device/a6cb2bc6443ed9be/wheather";
const char* mqttGPSTopic="device/a6cb2bc6443ed9be/GPS";
...
void mqttCallback(char* topic, byte* payload, unsigned int length) {
  ...
  else if(String(topic)==mqttcommendTopic&& messageTemp == "wheather")
  {
    wheather_flag=1;
   
    Serial.println("收到天气请求");
  }
}
...
//--------------------------------获取天气---------------------------------
void Get_API(void *pvParameters) {
  while(1) {
    if(wheather_flag==1)
    {
      String my_weather_array[3];
      wheather_flag=0;
      if (Get_Tx_API(my_weather_array,3)) { // 检查 Get_Tx_API 是否成功
        // 将数组内容格式化为一个单独的字符串
        String payload_string = my_weather_array[0] + "," + my_weather_array[1] + "," + my_weather_array[2];
        
        Serial.print("Publishing to MQTT: ");
        Serial.println(payload_string);

        // 使用 .c_str() 将 String 对象转换为 const char*
        if (mqttClient.connected()) {
          mqttClient.publish(mqttwheatherTopic, payload_string.c_str());
        } else {
          
        }
      } else {
        Serial.println("Failed to get weather data, not publishing.");
      }
    }
...
void dhtTask(void *pvParameters) {
  ...
  while(1){
    ...
    vTaskDelay(pdMS_TO_TICKS(2000)); // 每2秒读取一次并发布
  }
}
void GPS_Task(void *pvParameters) {
  setupSerial2(9600,RX2_PIN, TX2_PIN);
  Serial.println("串口2初始化完成");
  while(1) {
    char msgBuffer[100];
    getCompleteSerial2Message(msgBuffer,100);

    Serial.println(msgBuffer);
   mqttClient.publish(mqttGPSTopic, msgBuffer);
    vTaskDelay(pdMS_TO_TICKS(300)); // 每1秒读取一次并发布
  }
}
void setup() {
  pinMode(36,OUTPUT);
  ...
  {
    Serial.println("GPS任务创建成功");
  }
  else
  {
    Serial.println("GPS任务创建失败");
  }
  if(Get_Tx_Api!=NULL)
  {
    Serial.println("Get_Tx_Api任务创建成功");
  }
  else 
  {
    Serial.println("Get_Tx_Api任务创建失败");    
  }
  if(SmogHanle!=NULL)
  {
